...
CLASS_WEIGHTS = {
    "Nenhuma": np.float64(1.61),
    "Leve": np.float64(0.51),
    "Moderado": np.float64(0.41),
    "Severo": np.float64(0.86)
}

THRESHOLD = 0.60  # Threshold fixo
...
def predict_with_weights(model, tokenizer, texts, temperature=1.0):
    model.eval()
    predictions = []
    for text in texts:
        ...
        weighted_probs = np.array([probs[i] * CLASS_WEIGHTS[cls] for i, cls in enumerate(CLASSES)])
        if weighted_probs.max() >= THRESHOLD:
            pred_class = CLASSES[np.argmax(weighted_probs)]
        else:
            pred_class = "Nenhuma"
...
def main():
    parser = argparse.ArgumentParser(description="Treinamento BERTimbau multiclasse TOX com calibração de temperatura")
    ...
    output_dir_task = os.path.join(args.output_dir, "bertimbau_tox")
    ...
    preds = predict_with_weights(model, tokenizer, texts, temperature=T_value)
