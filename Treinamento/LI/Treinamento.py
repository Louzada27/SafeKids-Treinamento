...
def compute_metrics(pred):
    ...
    for i, cls in enumerate(CLASSES):
        per_class_metrics[cls] = {"precision": precision[i], "recall": recall[i], "f1": f1[i], "support": support[i]}
        weighted_precision += precision[i] * support[i]
        weighted_recall += recall[i] * support[i]
        weighted_f1 += f1[i] * support[i]
    metrics = {"accuracy": acc, "precision_mean": weighted_precision / total_support, 
               "recall_mean": weighted_recall / total_support, "f1": weighted_f1 / total_support}
...
# -------------------
def main():
    parser = argparse.ArgumentParser(description="Treinamento BERTimbau multiclasse com calibração de temperatura")
